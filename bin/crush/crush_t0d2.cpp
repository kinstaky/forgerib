...
constexpr int kT0d2CrusherNum = 9;
constexpr int kT0d2Modules[kT0d2CrusherNum] = {0, 1, 2, 3, 4, 5, 6, 7, 8};
constexpr int kT0d2Rates[kT0d2CrusherNum] = {250, 250, 250, 250, 250, 250, 250, 250, 250};

bool MapT0d2Event(const forgerib::DecodeEvent &decode, forgerib::RawDssdEvent &dssd) {
	...
	case 8:
		dssd.side = forgerib::kDssdSideBack;
		dssd.strip = decode.channel + (decode.module - 5) * 16;
		break;
	case 6:
...
int CrushT0d2(
	...
	TString filename = TString::Format("%s/t0d2_%04d.root", output_path, run);
	...
	if (report) {
		printf("Crushing T0D2   0%%");
		fflush(stdout);
	}
	std::vector<int> module(kT0d2Modules, kT0d2Modules + kT0d2CrusherNum);
	std::vector<int> rate(kT0d2Rates, kT0d2Rates + kT0d2CrusherNum);
	forgerib::Crusher crusher(kT0d2CrusherNum, run, 0, module, rate, raw_path, raw_prefix);
	for (
...
int main(int argc, char **argv) {
	cxxopts::Options options("crush_t0d2", "crush T0D2 detector ROOT output");
	options.add_options()
		("h,help", "Print usage")
		(
			"c,config",
