...
int main(int argc, char **argv) {
	cxxopts::Options options("map_vme", "map VME ROOT file to forged detector ROOT files");
	options.add_options()
		("h,help", "Print usage")
		(
			"v,vme_run",
	...
	const std::string workspace = RequireKey(tbl, "workspace");
	const std::string vme_path = RequireKey(tbl, "vme_path");
	const std::string vme_prefix = RequireKey(tbl, "vme_prefix");
	...
	TString tafd_path = TString::Format("%s/forge/tafd_vme_%04d.root", workspace.c_str(), vme_run);
	...
	TString t0csi_path = TString::Format("%s/forge/t0csi_vme_%04d.root", workspace.c_str(), vme_run);
	...
	TString tafcsi_path = TString::Format("%s/forge/tafcsi_vme_%04d.root", workspace.c_str(), vme_run);
	...
	TString beam_path = TString::Format("%s/forge/beam_vme_%04d.root", workspace.c_str(), vme_run);
	TFile beam_file(beam_path, "recreate");
	TTree beam_tree("tree", "beam vme");
	BeamEvent beam_event;
	SetupOutput(&beam_tree, beam_event);

	TString ppac_path = TString::Format("%s/forge/ppac_vme_%04d.root", workspace.c_str(), vme_run);
