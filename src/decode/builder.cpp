#include "include/decode/builder.h"
#include <cmath>

namespace glimmer {

Builder::Builder(
	const int num,
	const int run,
	const std::vector<int> &module,
	const std::vector<int> &rate,
	const char *raw_path,
	const char *prefix
) {
	total_size_ = 0;
	for (int i = 0; i < num; ++i) {
		readers_.push_back(std::make_unique<RawReader>(
			raw_path, prefix, run, module[i], rate[i]
		));
		events_.push_back(DecodeEvent());
		events_[i].used = true;
		total_size_ += readers_[i]->FileSize();
		module_finish_.push_back(false);
	}
	finished_num_ = 0;
	read_size_ = 0;
	last_percentage_ = 0;
}



int Builder::GetEvent(
	std::vector<DecodeEvent> &result,
	const double window,
	bool report
) {
	if (report && read_size_ * 100 / total_size_ > last_percentage_) {
		last_percentage_ = read_size_ * 100 / total_size_;
		printf("\b\b\b\b%3lu%%", last_percentage_);
		fflush(stdout);
	}
	result.clear();
	if (finished_num_ >= int(readers_.size())) return 0;
	bool updated = true;
	while (finished_num_ < int(readers_.size()) && updated) {
		updated = false;
		for (size_t m = 0; m < readers_.size(); ++m) {
			if (events_[m].used && !module_finish_[m]) {
				int bytes = readers_[m]->Read(events_.data()+m);
				if (bytes >= 16) {
					read_size_ += bytes;
				} else {
					module_finish_[m] = true;
					finished_num_ += 1;
				}
			}
		}
		if (finished_num_ == int(readers_.size())) return int(result.size());

		// find most early event
		if (result.size() == 0) {
			double min_time = -1.0;
			size_t min_index = 100;
			for (size_t m = 0; m < readers_.size(); ++m) {
				if (events_[m].used) continue;
				// std::cout << "  Module " << module[i] << ", ts " << events[i].timestamp << "\n";
				if (min_time < 0 || events_[m].time < min_time) {
					min_time = events_[m].timestamp;
					min_index = m;
				}
			}

			if (min_time < 0) return -1;

			// put first event
			result.push_back(events_[min_index]);
			events_[min_index].used = true;
			updated = true;
		} else {
			for (size_t m = 0; m < readers_.size(); ++m) {
				if (events_[m].used) continue;
				if (fabs(events_[m].time - result[0].time) < window) {
					result.push_back(events_[m]);
					events_[m].used = true;
					updated = true;
				}
			}
		}
	}
	return int(result.size());
}

}