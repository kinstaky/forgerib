...
#include "include/event/smelt/silicon_event.h"
...
int SmeltSilicon(
	...
	if (trigger_time.empty()) {
		return SmeltWithoutTrigger(path, output_path, title, window, report);
	}
	return SmeltWithTrigger(trigger_time, path, output_path, title, window, report);
}
...
int SmeltT0s(
	...
	return SmeltSilicon(trigger_time, path, output_path, "forged t0s", window, report);
}
...
int SmeltT1su(
	...
	return SmeltSilicon(trigger_time, path, output_path, "forged t1su", window, report);
}
...
int SmeltT1sd(
	...
	return SmeltSilicon(trigger_time, path, output_path, "forged t1sd", window, report);
}
